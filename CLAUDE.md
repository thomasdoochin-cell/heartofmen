...
- **Waitlist CTAs** ("I'm In", 5 of them — hero, bridge, what-it's-not, pricing, testimonials): each is preceded by a `.cta-closed` line ("Doors are closed for 2026. Join the waitlist for our next cohort."). Email box + button, submits to **Formspree** (`f/myezwzjo`) via `fetch` (no page redirect), then opens **`/waitlist?email=<their address>`** in a new tab and swaps in an inline success message. Someone who finishes both steps produces **two** Formspree entries (bare email, then full answers) — that's deliberate, so a bail-out still leaves you the address. *(Sept 2026: these were enrollment CTAs posting to `f/meevlybn` → Typeform `TingvB3U`; both are now unused.)*
...
- **Countdown bars** (top + bottom, black w/ gold): live Days/Hours/Minutes/Seconds computed in JS toward a target date. **Currently single-phase** (the Aug 2026 price-increase promo is over and its Phase-1 block was removed): lead `Enrollment closes in:` (hard-coded in the two `.cd-lead` spans in HTML), target `new Date(2026, 8, 21)` = **12:00am Mon Sept 21, i.e. midnight at the END of Sunday Sept 20**, local time (month is 0-indexed!). **Once the target passes, both `.countdown-bar`s are removed from the DOM** (on load if already expired, or live at zero if the page is open) — it no longer rolls to next year. After removal it fires a `resize` event so `nav.js` drops the hamburger back to its default `top` (nav.js offsets the toggle below the top bar). The owner's plan after this deadline: swap sign-up copy for **waitlist** copy. To bring a countdown back, set a new future date — the bars are still in the HTML. The bars also carry a `.countdown-sub` line under the timer — currently **"Only 2 Seats Left."** (uppercase, gold `<strong>`); it lives in the HTML of **both** bars, so edit both. `nav.js` measures the top bar and offsets the hamburger, so a taller bar repositions the toggle automatically — but check the **mobile hero CTA stays above the fold** after any height change (`.hero` mobile padding is the lever). Update the day integer to change the deadline — and note `new Date(y, m, d)` is the **start** of day `d`, so "closes midnight Sunday the 20th" means day **21**. When the owner gives a "midnight [day]" deadline, confirm start vs. end of that day; update the `.cd-lead` HTML if the copy changes.
  - *History/pattern:* we've previously run a **two-phase** `currentPhase()` version (a promo countdown that auto-flipped copy + date to the enrollment countdown once its deadline passed). If you need a temporary promo countdown again, reintroduce that pattern; use an **explicit-offset ISO string** like `new Date('2026-08-24T03:00:00-04:00')` for a fixed instant across all viewers (August is **EDT −04:00**; EST months use −05:00).
...
- **Nav & socials:** the hamburger menu (`nav.js`) lists Living Leadership, Full Circle Fund, and the Instagram + Substack icon links. The footer `.footer-social` row also has **Instagram** + **Substack** (`https://heartofmen.substack.com/`).
- **Sunday Saunter pop-up: lives in `nav.js`, not `index.html`** (moved Sept 21 2026) so every page loading nav.js gets it — currently the landing page and Full Circle Fund. Markup, styles (`.hom-popup-*`, hex not CSS vars, since other pages lack the `:root` block) and logic are all in the nav.js IIFE. Fires **15 s** after load, once per visitor (localStorage `hom_saunter_popup_v2` — note the **v2**; bumping the key re-shows it to everyone). `POPUP_ENABLED` at the top of that module switches it off. Header "Join our beloved Sunday Tradition", the Saunter newsletter form (`f/xoealrwk`), image `Images/sunday-saunter-header.jpg` (**3:1** — `.hom-popup-photo` uses `aspect-ratio: 2100/700`, so a replacement must match or it crops). *(Previous history: the old in-page version was)* — `const POPUP_ENABLED = false;` at the top of its IIFE in `index.html`. Markup, CSS and image are untouched; flip the flag to `true` to bring it back. While off it never sets the localStorage key, so re-enabling shows it to everyone again. When on, it fires **13 seconds** after load, **first visit only** (localStorage key `hom_saunter_popup_v1`). To re-test: Incognito, or `localStorage.removeItem('hom_saunter_popup_v1')`. Current graphic: `Images/Live-your-Genius-Sept-17.jpg` (1200×600 — the `.popup-photo` frame is fixed at **2:1** with `object-fit: cover`, so replacement graphics should be 2:1 or they crop). "In these 90 minutes" in the pop-up body links to the Luma RSVP (`https://luma.com/vi68x32g`, new tab), gold via `.popup-body a`.
...
- **`PRICE_ID`** (constant at the top of the function) is the deposit's `price_…`, a **one-time $2,250** payment → `mode: 'payment'`. Current live value: `price_1TyyGiRoQXKqP1qiSDdNcyyw`. As of Sept 2026 the **public** landing-page pricing **matches** Stripe: **$2,250 deposit + $775/mo** (Launch Support tier: $2,250 + $1,675/mo), all "for 8 months" starting Sept 30. (Aug 2026 briefly ran public at $2,750 + $975/$1,825 while Stripe intentionally stayed at $2,250; that gap is now closed.) Prices appear only in the two `.price-card`s in `index.html`. If the price is ever recurring, switch `mode` to `'subscription'`. **To test cheaply**, temporarily swap in a $1 price (we used `price_1U7N8bRoQXKqP1qiiTFX6fA2`), deploy, run a card, then swap the real price back — leave a `// TEMP` comment so the revert isn't forgotten. Price ID must match the key mode (live price with live keys).
