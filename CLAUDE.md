...
- **Sunday Saunter pop-up:** fires **13 seconds** after load, **first visit only** (localStorage key `hom_saunter_popup_v1`). To re-test: Incognito, or `localStorage.removeItem('hom_saunter_popup_v1')`. Current graphic: `Images/Live-your-Genius-Sept-17.jpg` (1200×600 — the `.popup-photo` frame is fixed at **2:1** with `object-fit: cover`, so replacement graphics should be 2:1 or they crop). "In these 90 minutes" in the pop-up body links to the Luma RSVP (`https://luma.com/vi68x32g`, new tab), gold via `.popup-body a`.
